...
>
> **层叠约定（强制）**：页面内吸顶导航 `z-50` < 全局 Header `z-[55]`（下拉菜单在 Header 内弹出，须高于页面吸顶条）< 模态弹窗 `z-[60]` < 图片预览 `z-[9999]`。新增 fixed/sticky 元素时对照本梯度取值，禁止随意写高 z-index。
...
```
[买家说 logo→首页] [🏠 首页] [💬 消息] [(居中)搜索] [🌙 主题] [审核台·管理员] [+ 发布] [头像+昵称 / 登录]
```
...
- **+ 发布**（coral 主 CTA）紧邻身份区左侧；**身份区为导航最右锚点**：已登录 = 头像+昵称（点击展开**下拉菜单**：我的主页 / 编辑资料 / 修改密码 / 退出登录（二次确认弹窗，Esc 关闭）；菜单点击外部 / Esc 关闭）；未登录 = 「登录」按钮（ghost，`/login?redirect=当前路径`）。
- **右操作区双层分组（强制）**：工具组（主题 / 审核台）与操作组（发布 + 身份区）以 **16px** 分隔，组内 **8px**——主 CTA 与身份区成组紧邻，工具按钮不与 CTA 混列；Feed 导航与 DesktopHeader 必须完全一致（含发布按钮高度 40px）。
- 键盘：`Esc` = 返回上一页（弹窗打开时豁免）；logo 与「首页」回首页。
- logo 必须可点击（button + `aria-label="返回首页"`）。
- 登录态同步：导航随路由变化重新获取用户信息（常驻组件不重挂载）。
...
### 4.4 吸顶与滚动行为
- **个人主页 Tab 吸顶**：分享/收藏/赞过（本人主页另含「评论」，仅本人可见）`sticky top-14`（紧贴 Header / 移动导航下），资料卡随滚动移出。
